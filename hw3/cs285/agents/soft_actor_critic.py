...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if num_critic_networks > 1:
            if self.target_critic_backup_type == "doubleq":
                ...
                next_qs = torch.min(next_qs, dim=0).values.unsqueeze(0).repeat(num_critic_networks, 1)
            elif self.target_critic_backup_type == "mean":
                next_qs = torch.mean(next_qs, dim=0).values.unsqueeze(0).repeat(num_critic_networks, 1)
            else:
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            target_values: torch.Tensor = reward + self.discount*(1-done.to(torch.int8))*next_qs
    ...
    def update(
        ...
        for _ in range(self.num_critic_updates):
            critic_infos.append(self.update_critic(obs=observations, action=actions, next_obs=next_observations, reward=rewards, done=dones))
        ...
        if self.soft_target_update_rate is None and step % self.target_update_period == 0:
            self.update_target_critic()
